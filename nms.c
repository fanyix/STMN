...
void compute_iou(THFloatTensor *result, THFloatTensor *a, THFloatTensor *b)
{
  ...
  // a = THTensor_(newContiguous)(a);
  // b = THTensor_(newContiguous)(b);
  // result = THTensor_(newContiguous)(result);
  ...
  for(int i=0;i<Na;++i)
  {
  	for(int j=0;j<Nb;++j)
  	{
  		float *a_cursor = a_data + i*4;
  		float *b_cursor = b_data + j*4;
	    r_data[i*Nb+j] = overlap(a_cursor, b_cursor);
	}
  }
}
