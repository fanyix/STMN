function [paths, frames] = zero_jump_link(frames, downweight_notrack)
% AUTORIGHTS
% ---------------------------------------------------------
...
% link_thresh = 0.8;
% link_weight = 0.5;
% score = double(get_overlap_MtoN(v1.boxes, v2.boxes) > link_thresh) * link_weight;

for i1=1:N1
    % scores
    scores2 = v2.scores;
    scores1 = v1.scores(i1);
    score(i1,:) = score(i1,:) + scores1+scores2';
end
