function [paths, remainBoxes] = make_tubes(frameBoxes,max_per_image,box_voting, tracks_cell, downweight_notrack)
if nargin < 2
    max_per_image = 200;
end
if nargin < 3
...
nms_th = 0.4; % default to 0.4
do_box_rescoring = false; % default to false
...
for f = 1:length(frameBoxes)
  ...
  if box_voting
      boxes = nms_box_voting(boxes, ...
               ...
               'do_bbox_voting',box_voting, ... 
               'add_val',0.0, ...
               'use_gpu',false, ...
               'do_box_rescoring', do_box_rescoring);
      boxes = boxes{:};
  else
      nms_idx = nms(single(boxes(:,1:5)), nms_th); 
...
%%---------- zero_jump_link ------------
[paths, object_frames] = zero_jump_link(object_frames, downweight_notrack);
remainBoxes = arrayfun(@(x)[x.boxes, x.scores], object_frames, 'uniformoutput', false);
