function bbox_detections_per_class = nms_box_voting(bbox_cand_dets, varargin)
...
for j = 1:num_classes
    ...
    % choose between NMS or SoftNMS
    % 1. NMS
    bbox_dets = apply_nms(bbox_cand_dets_this_class, thresholds(j), nms_iou_thrs, max_per_image, use_gpu);
    % % 2. our implementation of softNMS
    % bbox_dets = apply_soft_nms(bbox_cand_dets_this_class, thresholds(j), 0.5, 1e-6, max_per_image);
    % % 3. Marvin's implementation of softNMS
    % bbox_dets = soft_nms_marvin(bbox_cand_dets_this_class, -1, 0.5, 2);
    % [ignore, sortidx] = sort(bbox_dets(:, 5), 'descend');
    % bbox_dets = bbox_dets(sortidx, :);
    % val_num = sum(bbox_dets(:,5)>1e-5);
    % val_num = min(max(val_num, 5), size(bbox_dets, 1));
    % bbox_dets = bbox_dets(1:val_num, :);
...
function [bbox_dets, indices] = apply_soft_nms(bbox_det_cands, score_thresh, sigma, soft_nms_score_thresh, max_per_image)
bbox_dets = zeros(0, 5, 'single');

if ~isempty(bbox_det_cands)
    indices = find(bbox_det_cands(:,5) > score_thresh);
    if ~isempty(indices)
        bbox_det_cands    = soft_nms(bbox_det_cands(indices, :), sigma, soft_nms_score_thresh);
        [~, order] = sort(bbox_det_cands(:,5), 'descend');
        order      = order(1:min(length(order), max_per_image));
        bbox_dets  = bbox_det_cands(order,:);
    end
end
end
...
for p = 1:num_dets
    ...
    % this is for rescoring
    % rescore_neighbors_mask = overlap >= 0.85;
    % rescore_bbox_neighbors = bbox_dets_cands(rescore_neighbors_mask,:); 
    % rescore_bbox_neighbors(:,5) = eps + max(0, rescore_bbox_neighbors(:,5) + add_val); 
    % if any(rescore_neighbors_mask)
    %     bbox_dets(p,5)  = sum(rescore_bbox_neighbors(:,5) .* overlap(rescore_neighbors_mask,:)) / ...
    %                         sum( overlap(rescore_neighbors_mask,:));
    % end
