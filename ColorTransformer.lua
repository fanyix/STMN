...
require 'image'
local M = torch.class('fbcoco.ColorTransformer')

function M:__init(brightness_var, contrast_var, saturation_var, lighting_var, pca)
  self.pca = pca or {
  ...
  self.brightness_var = brightness_var or 0.4
  self.contrast_var = contrast_var or 0.4
  self.saturation_var = saturation_var or 0.4
  self.lighting_var = lighting_var or 0.1
  
  self:SampleColorParams()

  self.operator = {
    self:ColorJitter(), self:Lighting()
  }
   
end

function M:forward(input)
  for _, operator in ipairs(self.operator) do
...
function M:SampleColorParams()
  self.order_x = torch.randperm(3)
  self.lighting_x = torch.Tensor(3):normal(0, self.lighting_var)
  self.brightness_x = torch.uniform(-self.brightness_var, self.brightness_var)
  self.contrast_x = torch.uniform(-self.contrast_var, self.contrast_var)
  self.saturation_x = torch.uniform(-self.saturation_var, self.saturation_var)
end
...
function M:TenCrop(size)
   local centerCrop = self:CenterCrop(size)
...
function M:RandomSizedCrop(size)
   ...
   local crop = self:CenterCrop(size)
...
function M:Lighting()
   return function(input)
      local rgb = self.pca.eigvec:clone()
         :cmul(self.lighting_x:view(1, 3):expand(3, 3))
         :cmul(self.pca.eigval:view(1, 3):expand(3, 3))
         :sum(2)
         :squeeze()
      input = input:clone()
      for i=1,3 do
...
function M:Saturation()
   local gs

   return function(input)
      gs = gs or input.new()
      grayscale(gs, input)
      local alpha = 1.0 + self.saturation_x
...
function M:Brightness()
   ...
   return function(input)
      ...
      local alpha = 1.0 + self.brightness_x
      blend(input, gs, alpha)
      return input
   end
end
...
function M:Contrast()
   ...
   return function(input)
      ...
      grayscale(gs, input)
      gs:fill(gs[1]:mean())
      local alpha = 1.0 + self.contrast_x
      blend(input, gs, alpha)
      return input
   end
end
...
function M:RandomOrder(ts)
   return function(input)
      ...
      for i=1,#ts do
         local img, tmp = ts[self.order_x[i]](img)
      end
      return img
   end
end
...
function M:ColorJitter()
   local ts = {}
   table.insert(ts, self:Brightness())
   table.insert(ts, self:Contrast())
   table.insert(ts, self:Saturation())
