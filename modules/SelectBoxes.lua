...
function SelectBoxes:updateOutput(input)
   ...
   local ys = input[2]
   assert(torch.type(classes) == 'torch.CudaTensor', 'nn.SelectBoxes only has GPU implementation now')

   local B = classes:size(1)
   self.maxvals = self.maxvals or classes.new()
   self.maxids = self.maxids or torch.CudaLongTensor()
   ...
   maxids = maxids:type('torch.CudaTensor')
