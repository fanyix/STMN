...
function DataSetCOCO:create(name, roidbfile, nsamples, data_root)
   local dataset
   dataset = DataLoader(name, data_root)
...
function DataSetCOCO:attachProposals(i)
   ...
   local boxes = self:getROIBoxes(i):clone()
   
  -- NOTE: This is for imagenet2014
  local hgt = self.dataset.images.height[i]
  local wid = self.dataset.images.width[i]
  local scaler = boxes.new({wid, hgt, wid, hgt}):view(1, 4)
  boxes:cmul(scaler:expandAs(boxes))
